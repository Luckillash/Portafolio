...
function Galaxy({ dof }) {
  const parameters = useControls({
    Galaxy: folder({
      count: { min: 100, max: 1000000, value: 50000, step: 100 }, // Default value is= 100000
      ...
      insideColor: { value: '#ff6130', label: 'Inside Color' },
  ...
  const generateGalaxy = () => {
    ...
    const colorInside = new THREE.Color(0.6, 0.1765, 0.3882)
...
export default function App() {
  ...
  return (
    <Canvas performance={{ min: 0.1 }} style={{ position: "absolute"}} linear flat camera={{ position: [0, 2, 2] }}>
      <Suspense fallback={null}>
        <Galaxy dof={dof} />
        {/* <Nucleus size={0.125} /> */}
      </Suspense>
